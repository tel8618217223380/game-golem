...
*/	},{
		...
		id:'choice',
		label:'Attack',
		select:['Any', 'Strongest', 'Weakest', 'Shortest', 'Spread', 'Max_Damage', 'Min_Damage']
	},{
		id:'stop',
		label:'Stop',
...
Monster.update = function(what) {
	...
	for (i in this.data) {
            ...
            for (j in this.data[i]) {
                if (((!this.data[i][j].health && this.data[i][j].state === 'engage') || typeof this.data[i][j].last === 'undefined' || this.data[i][j].last < (Date.now() - 3600000)) && (typeof this.data[i][j].ignore === 'undefined' || !this.data[i][j].ignore) && (this.data[i][j].state !== 'complete')) {
		...
		if ((typeof this.data[i][j].ignore === 'undefined' || !this.data[i][j].ignore) && this.data[i][j].state === 'engage' && this.data[i][j].finish > Date.now() && (this.option.ignore_stats || (Player.get('health') >= this.runtime.health && Queue.burn.stamina >= this.runtime.stamina ))) {
                    ...
                    if (this.data[i][j].name === 'You' && this.option.own){
                        ...
                        list.push([i, j, this.data[i][j].health, this.data[i][j].eta, this.data[i][j].battle_count,((sum(this.data[i][j].damage[userID]) || 0) / this.data[i][j].damage_total * 100).round(1),this.data[uid][type].totaldefense || 0]);
                        break;
                    } else {
                        switch(this.option.stop) {
                            default:
                            case 'Never':
                                list.push([i, j, this.data[i][j].health, this.data[i][j].eta, this.data[i][j].battle_count,((sum(this.data[i][j].damage[userID]) || 0) / this.data[i][j].damage_total * 100).round(1),this.data[uid][type].totaldefense]);
                                break;
                            case 'Achievement':
